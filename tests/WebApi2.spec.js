...
test.beforeAll(async ({ browser }) => {
  ...
  //Captures and saves the current browser session to a file called state.json;
  //Includes: Cookies, localStorage, sessionStorage, and other browser state
  //Since we have to store the cookies of the browser in entirety, we have called storageState() on context i.e at browser level, instead page level
  await context.storageState({ path: "state.json" });

  //Creates another new browser context with the same saved session state
  //Stores it in the webContext variable for use in tests
